...
*/
int ExtremeRandomlySplitOnDLoquatNode(float **data, int samples_num, int variables_num, int *innode_samples_index, int innode_num, int &split_variable_index, float &split_value)
{
	int j, index;
										...
										int* innode_samples_index, int innode_num, int Mvariable, int& split_variable_index, float& split_value)
{
	...
	for (j = 0; j < Mvariable; j++)
	{
		...
		// 排序
		qsort(vls, innode_num, sizeof(var_label), _cmp);
		...
		for (k = 1; k < innode_num; k++)
		{
			if (abs(vls[k - 1].var - vls[k].var) < FLT_EPSILON)
			{
				continue;
			}

			splitv = 0.5f * (vls[k - 1].var + vls[k].var);
			// calc gini
			lcount = k;
			rcount = innode_num - k;
			lgini = 0.f;
...
int AnalyzeTrainingSamplesArrivedAtOneNode2(const int* label, int samples_num, int classes_num, const int* innode_samples_index, int innode_num, float& impurity, float*& class_distribution)
{
	if (innode_num == 0)
	{
		cout << endl;
...
struct LoquatCTreeNode* GrowLoquatCTreeNodeRecursively(float **data, int *label, int *&sample_arrival_index, int arrival_num,  GrowNodeInput *pInputParam, struct LoquatCTreeStruct *loquatTree)
{
...
void _DisplayLoquatTreeInfo(struct LoquatCTreeStruct *loquatTree, RandomCForests_info RFinfo)
{
	...
	for( i=0; i<=tree_depth; i++ )
	{
		...
		for( j=0; j<nodes_num; j++ )
		{
			...
			if( pNode[j]->nodetype == enLinkNode || pNode[j]->nodetype == enRootNode )
			{
			...
			if( pNode[j]->nodetype == enLeafNode )
			{
				pNextNode[j*2]   = NULL;
				pNextNode[j*2+1] = NULL;
			}else
...
void DisplayLoquatTreeInfo(struct LoquatCTreeStruct* loquatTree, RandomCForests_info RFinfo)
{
	...
	while (treeNodes.size()>0)
	{
		...
		for (; it != treeNodes.end(); it++)
		{
			...
			if (pNode->nodetype == enLinkNode || pNode->nodetype == enRootNode)
			{
